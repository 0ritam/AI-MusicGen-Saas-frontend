...
export async function getPlayUrl(songId: string) {
  ...
  // First check if song exists and user has access
  ...
  if (!song) {
    throw new Error("Song not found or you don't have permission to access it");
  }

  console.log('Song debug info:', { id: songId, s3Key: song.s3Key, status: song.status });

  if (!song.s3Key) {
    if (song.status === "processing") {
      throw new Error("Song is still being generated. Please wait a moment and try again.");
    } else if (song.status === "failed") {
      throw new Error("Song generation failed. Please try generating a new song.");
    } else if (song.status === "no credits") {
      throw new Error("Song generation failed due to insufficient credits.");
    } else if (song.status === "queued") {
      throw new Error("Song is queued for generation. Please wait a moment and try again.");
    } else {
      throw new Error("Song audio is not ready yet. Please try again later.");
    }
  }
